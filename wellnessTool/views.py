...
def questionnaire(request):
    if request.method == "POST":
        ...
        if Choice.objects.filter(
            date_submit=date.today().strftime("%Y-%m-%d")
        ).exists():
            submit_today = True
        else:
            submit_today = False
        return render(
            request, "questionnaire.html", {"form": form, "submit_today": submit_today}
        )


def stats(request):
    q1_answers_list = []
    for answer in Choice.objects.values_list("choice_num", flat=True).filter(
        question_id=1
    ):
    ...
    if len(q1_answers_list) != 0:
        ...
        for item in date_submitted:
            wellness_score_list.append(
                sum(
                    Choice.objects.values_list("choice_num", flat=True).filter(
                        date_submit=item
                    )
                )
            )
        ...
        wellness_score_check_sd = round(
            abs(wellness_score_previous_average - wellness_score_today), 1
        )
